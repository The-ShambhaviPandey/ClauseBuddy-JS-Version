...
export default function Uploads() {
  ...
  const handleDeleteFile = async (fileId) => {
    try {
      const response = await fetch(
        `http://localhost:5000/api/chat/files/${fileId}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const queryGemini = async (query) => {
    const res = await fetch(
      "http://localhost:8000/api/gemini-answer",
      {
        method: "POST",
        ...
        {files.length === 0 ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {files.map((file) => (
              <Card
                key={file._id}
                className="bg-gray-900/60 border-purple-900/30 hover:bg-gray-900/80 transition-colors overflow-hidden"
              >
                <CardContent className="p-0">
                  ...
                  <div className="p-3 space-y-2">
                    <div className="flex items-center justify-between">
                      <p
                        className="text-white text-sm font-medium truncate mr-2"
